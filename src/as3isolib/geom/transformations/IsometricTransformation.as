package as3isolib.geom.transformations
{
	...
	{
		public function IsometricTransformation (angleOfProjection:Number = 30)
		{
			cosTheta = Math.cos(angleOfProjection * Math.PI / 180);
			sinTheta = Math.sin(angleOfProjection * Math.PI / 180);
		}
		...
		public function screenToSpace (screenPt:Pt):Pt
		{
			...
			var y:Number = -1 * (screenPt.x / 2 * cosTheta) + screenPt.y + screenPt.z;
			var x:Number = (screenPt.x / 2 * cosTheta) + screenPt.y - screenPt.z;//screenPt.x / 2 + screenPt.y + screenPt.z;
